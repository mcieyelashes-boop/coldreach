...
export default function IntegrationsPage() {
  ...
  async function importHubSpot() {
    if (!hubspotToken) { setHubspotMsg('Enter your HubSpot Private App token'); setHubspotStatus('error'); return }
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Failed to fetch HubSpot contacts')
      storage.addContacts(data.contacts)
      setHubspotStatus('success')
      setHubspotMsg(`Imported ${data.contacts.length} contacts from HubSpot`)
    } catch (e: unknown) {
      setHubspotStatus('error')
      setHubspotMsg(e instanceof Error ? e.message : 'Unknown error')
    }
  }
  ...
  const StatusBadge = ({ status, msg }: { status: Status; msg: string }) => {
    ...
    const colors = { loading: 'bg-blue-50 text-blue-700 border-blue-200', success: 'bg-emerald-50 text-emerald-700 border-emerald-200', error: 'bg-red-50 text-red-700 border-red-200' }
    return (
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Google API Key <span className="text-slate-400">(for private sheets)</span></label>
              <input
                value={sheetsApiKey}
            ...
            <p className="text-xs text-slate-400">Sheet must have columns: email, first_name, last_name, company, title (header row)</p>
            <button
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Sync contacts from your CRM</p>
            </div>
          </div>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Private App Token</label>
              <input
                value={hubspotToken}
            ...
            <p className="text-xs text-slate-400">Create a Private App in HubSpot Settings → Integrations → Private Apps</p>
            <button
              onClick={importHubSpot}
              disabled={hubspotStatus === 'loading'}
              className="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors"
            >
              {hubspotStatus === 'loading' ? 'Syncing...' : 'Sync Contacts'}
            </button>
          </div>
          <StatusBadge status={hubspotStatus} msg={hubspotMsg} />
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Upload a contacts CSV file</p>
            </div>
          </div>
          <div className="space-y-3">
            <p className="text-xs text-slate-400">Required column: <code className="bg-slate-100 px-1 rounded">email</code>. Optional: <code className="bg-slate-100 px-1 rounded">first_name</code>, <code className="bg-slate-100 px-1 rounded">last_name</code>, <code className="bg-slate-100 px-1 rounded">company</code>, <code className="bg-slate-100 px-1 rounded">title</code></p>
